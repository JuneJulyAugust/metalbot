...
# Deployment script for metalbot-arduino (Pi-side)
# Usage: ./deploy.sh [pi-hostname-or-ip]

PI_HOST="${1:-pi}"
REMOTE_DIR="~/metalbot-arduino"
...
ssh "$PI_HOST" "
    ...
    export PATH=\"\$HOME/metalbot-arduino/tools/bin:\$PATH\" && \
    export ARDUINO_CONFIG_FILE=\"\$HOME/metalbot-arduino/tools/arduino-cli.yaml\" && \
    export ARDUINO_DIRECTORIES_DATA=\"\$HOME/metalbot-arduino/tools/arduino_data\" && \
    export ARDUINO_DIRECTORIES_USER=\"\$HOME/metalbot-arduino/tools/arduino_data/user\" && \
    export ARDUINO_DIRECTORIES_DOWNLOADS=\"\$HOME/metalbot-arduino/tools/arduino_data/staging\" && \
